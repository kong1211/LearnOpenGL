...
// ----------------------------------------------------------------------------
float GeometrySmith(vec3 norm, vec3 V, vec3 L, float roughness)
{
...
void main()
{
    ...
    float roughness = clamp(texture(roughnessMap, TexCoords).r, 0.05, 1.0);
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        vec3 specular = (NDF * G * F) /
                        (4.0 * max(dot(norm, V), 0.0) * max(dot(norm, L), 0.0) + 0.0001);
